...
router.get('/banner4',function(req,res){
    ...
    pool.query(`SELECT * from banner4`,function(err,rows){
        if(err) throw err;
        res.send(rows);
    })
});
...
var imgs;
router.post('/ban4img',function(req,res){
...
router.post('/upban4',function(req,res){
    var id=req.body['id']
    console.log(id)
    res.header("Access-Control-Allow-Origin", "*");
    pool.query(`update banner4 set src="${imgs}" where id=${id}`,function(err,rows){
        if(err) throw err;
        res.send('修改成功');
    })
});
