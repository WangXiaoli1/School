...
router.get('/banner3',function(req,res){
    ...
    pool.query(`SELECT * from banner3`,function(err,rows){
        if(err) throw err;
        res.send(rows);
    })
});
...
var imgs;
router.post('/ban3img',function(req,res){
...
router.post('/upban3',function(req,res){
    var id=req.body['id'];
    console.log(id);
    res.header("Access-Control-Allow-Origin", "*");
    pool.query(`update banner3 set src="${imgs}" where id=${id}`,function(err,rows){
        if(err) throw err;
        res.send('修改成功');
    })
});
